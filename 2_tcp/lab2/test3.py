...
print("SEQ: ", seq, "SEQ_NO", seq_no, "ACK_NO", ack_no, "ACK", ack, "PAYLOAD SIZE", len(payload))
assert seq == ack_no
...
for i in range(5):
    ...
    conexao.enviar(payload)
    print("Payload enviado", i, len(payload))
    ...
    for j in range(nseg):
        ...
        assert seq == ack_no
        assert (flags & FLAGS_ACK) == FLAGS_ACK and ack == seq_no
        print(segmento[4*(flags>>12):])
        print(payload[j*MSS:(j+1)*MSS])
        print(i, j)
